...
def our_names():
    """Country code -> display name from the authoritative registry."""
    return {code: values[0] for code, values in COUNTRIES.items()}
...
def main():
    ...
    for name, boxlist in members.items():
        ...
        # Antarctica spans both sides of the antimeridian and legitimately
        # occupies the full longitude range. Shifting -180 to +180 collapses
        # that box to zero width, so retain a nearly global plain extent.
        plain_width = plain[3] - plain[2]
        cont[name] = plain if plain_width >= 359 or plain_width <= (shifted[3] - shifted[2]) else shifted
