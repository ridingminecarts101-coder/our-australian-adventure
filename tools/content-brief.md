...
| `pack` | `null` when `hidden_gem` is false. When true, the continent slug: `oceania` `europe` `north-america` `south-america` `asia` `middle-east` `africa` |
...
- **Fewer than one in five entries per `admin1`.** Four gems needs twenty-one
  entries in that region. Count as you go.
...
- Gems are under 1/5 per `admin1`, and every gem has a `pack`
