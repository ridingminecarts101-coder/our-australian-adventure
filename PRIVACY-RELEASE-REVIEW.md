...
`privacy.html` and `support.html` describe required accounts, optional completion sharing, individual purchases, the actual location provider, and an external deletion-request route. The owner supplied **help.rlapplications@gmail.com** as the working, monitored public mailbox for support, privacy, deletion requests and moderation; this implementation pass did not independently send or receive a message. The verified Resend no-reply transactional sender remains available for account messages but is not a monitored support route.
...
| External requests | `support.html#delete-account` is the proposed Play deletion URL. The owner says the published Gmail mailbox is working and monitored; independently exercise request handling and verify identity without requesting passwords. Agree a response/fulfilment time and record completion of provider cleanup. Do not claim immediate deletion everywhere. |
