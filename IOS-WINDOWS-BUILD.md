...
The exact-source unsigned run [34916632219](https://github.com/ridingminecarts101-coder/our-australian-adventure/actions/runs/34916632219) passed for commit `77d1a356a8b9aab7071ea4bba0aea3904ba73bbb`. Later source changes require another exact-head compile after they are reviewed and pushed. A successful simulator compile does not prove that an iPhone install, purchase or App Store submission works.
...
App Store Connect access for the owner's individual/sole-trader account was verified on 15 September 2026: the signed-in Apps page and Users and Access / Business areas load. The Apps page currently shows no apps, and the New App form has no Bundle ID available. Before the first signed run, create and verify the explicit App ID, record the selected Team ID and confirm the exact seller identity shown by Apple; the account menu name alone is not seller-field evidence. The owner accepts the individual/sole-trader identity for the initial release; RL Applications remains the studio brand, and a possible later Apple conversion is optional. RevenueCat project `1bce63b8` already has the eight exact entitlements and **Keep with original App User ID** saved. Its Apple app, Apple products and `appl_` public SDK key do not exist because app creation was rejected without the Apple IAP `.p8`, Key ID and Issuer ID. Obtain those privately, create the app and eight non-consumables from `tools/ios-products.json`, attach each product to its matching entitlement, and validate the credentials before running the signed workflow.
