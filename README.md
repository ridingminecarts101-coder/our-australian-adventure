...
supabase/
  ...
  schema-photos.sql photos table + private storage bucket (optional)
...
Photos live in a **private** Storage bucket. They are not readable by URL — the
app mints short-lived signed links for each view, so a leaked link expires rather
than exposing the album permanently. The trade-off is that photos need a
connection to view; everything else in the app works offline.
