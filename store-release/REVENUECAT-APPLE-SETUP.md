...
3. RevenueCat Apple app **Wayfinder iOS** (`679dd6fda1`) now exists for bundle ID `app.wayfinder.mobile`. Its separate Apple In-App Purchase key has been secured by the owner and RevenueCat reports **Valid credentials**. The App Store public SDK key exists and is stored as the protected workflow variable; RevenueCat's Test Store example products and key remain non-production fixtures and must not be shipped.
4. All eight Apple non-consumables now have verified en-AU localization, Australian base pricing and 175 current territories with automatic future-territory inclusion. **All continents** is AUD 14.99; all seven regional packs are AUD 2.99. Family Sharing remains off. Apple reports `MISSING_METADATA` for all eight until review screenshots and remaining submission metadata are completed; none has been submitted for review.
...
- App Store Connect access for the owner's individual/sole-trader account was verified on 15 September 2026. Team **`BJ764589GP`**, explicit App ID **`app.wayfinder.mobile`** and iOS app record **`6812170174`** now exist. The record uses **Wayfinder: Adventure Lists**, SKU **`wayfinder-ios-001`** and primary locale **en-AU**. The owner accepts Apple's individual-account developer/seller identity for the initial release. Keep RL Applications as studio branding and copy the exact seller string from its Apple field rather than treating the account menu name as proof. A possible later organisation conversion is optional and pending Apple's response.
- Paid Apps, banking and tax show **Active**. EU trader status is **In Review**; resolve it or make an explicit storefront decision before release. The app download is free; the eight guide unlocks are paid non-consumables.
- All eight Apple non-consumables have verified localization, base pricing and worldwide availability. Their review screenshots and remaining submission metadata are still missing. The RevenueCat Apple app, IAP credentials and `appl_` public SDK key are configured, but live purchase readiness still requires exact product mapping, a signed build and sandbox purchase/restore testing.
...
The active App Store Connect team key **Wayfinder Release Automation** has App Manager access. Its nonsecret identifiers are Key ID **`MPAR977934`** and Issuer ID **`5efc1572-51a2-4d32-9c45-14d353166059`**. The private `.p8` is secured locally with restricted ACLs and stored in the protected GitHub environment as `APP_STORE_CONNECT_API_PRIVATE_KEY_P8_BASE64`. Earlier keys `WAJL8DPTRF` and `DMLD9ZG9G3` are revoked and must not appear in active configuration. This upload key is distinct from RevenueCat's In-App Purchase key `J6Y6B6PY2N`.
...
Signing access is now prepared: Apple Distribution certificate `28TGSPJS28` and active App Store profile `237QZ6YF66` bind Team `BJ764589GP` to `app.wayfinder.mobile` through 15 September 2027. The encrypted P12, password and profile are stored only in the restricted local signing directory and the protected GitHub `app-store` environment. No signed workflow has been dispatched against the current source candidate; do that only after source/backend integration review.
