...
- App Store Connect access for the owner's individual/sole-trader account was verified on 15 September 2026: Apps, Users and Access, and Business load. There are no apps, and New App currently offers no Bundle ID. The owner accepts Apple's individual-account developer/seller identity for the initial release. Keep RL Applications as studio branding; create and verify the explicit App ID, record the selected Team ID, and copy the exact seller string from its Apple field rather than treating the account menu name as proof. A possible later organisation conversion is optional and pending Apple's response.
